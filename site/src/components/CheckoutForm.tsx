...
import { deliveryFee, formatPrice, missingForFreeShipping } from '../lib/order'
      ...
      <Block title="Entrega">
        <div className="rounded-2xl border border-acai-100 bg-acai-50/70 px-4 py-3">
          <div className="flex items-center justify-between gap-3 text-sm">
            <span className="font-semibold text-ink">Taxa de entrega</span>
            <span className={`font-extrabold ${fee === 0 ? 'text-green-700' : 'text-acai-800'}`}>
              {fee === 0 ? 'Grátis' : formatPrice(fee)}
            </span>
          </div>
          <p className="mt-1 text-xs text-muted">
            Chega em cerca de {business.delivery.averageMinutes} minutos depois da confirmação.
          </p>
          {missingForFree > 0 && (
